...
program.command('revoke')
  ...
  .action(async () => {
    try {
      ...
      console.error('Failed to revoke authentication:', error.message);
      process.exit(1);
    }
  });

program.command('permissions')
  .description('Display current authentication status and configured permissions')
  .action(async () => {
    try {
      ...
      if (config.enabledTools.length > 0) {
        ...
        console.log('No specific tools are explicitly enabled in config. All available tools will be used.');
      }
      console.log('----------------------------------');
    } catch (error) {
